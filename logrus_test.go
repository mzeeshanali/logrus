...
import (
	...
	. "github.com/datumbrain/logrus/internal/testutils"
)
...
func TestReportCallerWhenConfigured(t *testing.T) {
	...
	LogAndAssertJSON(t, func(log *Logger) {
		...
		assert.Equal(t, "info", fields["level"])
		assert.Equal(t,
			"github.com/datumbrain/logrus_test.TestReportCallerWhenConfigured.func3", fields[FieldKeyFunc])
	})
...
func TestNestedLoggingReportsCorrectCaller(t *testing.T) {
	...
	assert.Equal(t, "eating raw fish", fields["context"])
	assert.Equal(t,
		"github.com/datumbrain/logrus_test.TestNestedLoggingReportsCorrectCaller", fields["func"])
	cwd, err := os.Getwd()
